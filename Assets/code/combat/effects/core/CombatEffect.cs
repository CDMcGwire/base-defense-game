...
public abstract class CombatEffect : PayloadObject<CombatEffect> {
	...
	/// new effect values.
	/// </summary>
	/// <param name="mod">A <see cref="CombatMod"/> that should be applied to the effect.</param>
	/// <returns>A CombatEffect that has been modified according to its mod logic.</returns>
	public abstract CombatEffect Modify(CombatMod mod);

	/// <summary>
	/// Adds all mods in the given chain to the to the effect. This method modifies the effect in place.
	/// If the modification should be temporary, use AddTempMods.
	/// </summary>
